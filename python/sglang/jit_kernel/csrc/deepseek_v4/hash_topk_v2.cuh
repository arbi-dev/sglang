// Copy of hash_topk.cuh's MoEHashTopKParams + HashTopKKernel,
// templated on the input_id dtype so callers can pass either int32 or int64.
// The original hash_topk.cuh is left bitwise unchanged.
...
struct MoEHashTopKParams {
  ...
  const InputIdT* __restrict__ input_id;
...
template <typename InputIdT, auto Fn, bool kUsePDL>
__global__ void moe_hash_topk_fused(const MoEHashTopKParams<InputIdT> __grid_constant__ params) {
  ...
  const auto token_id = static_cast<int64_t>(input_id[warp_id]);
...
struct HashTopKKernel {
  static constexpr auto kernel = moe_hash_topk_fused<InputIdT, Fn, kUsePDL>;
  ...
  run(const tvm::ffi::TensorView router_logits,
    ...
    const auto params = MoEHashTopKParams<InputIdT>{
        .router_logits = static_cast<const float*>(router_logits.data_ptr()),
        .input_id = static_cast<const InputIdT*>(input_id.data_ptr()),
