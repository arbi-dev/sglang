    ...
    def alloc(self, need_size: int) -> Optional[torch.Tensor]:
        if need_size % self.page_size != 0:
            raise ValueError(
                ...
                f"got need_size={need_size} and page_size={self.page_size}"
            )
        if need_size > self.available_size():
            return None
        select_index = self.free_slots[:need_size]
    ...
    def free(self, indices: torch.Tensor) -> int:
        if len(indices) % self.page_size != 0:
            raise ValueError(
                ...
                f"got len(indices)={len(indices)} and page_size={self.page_size}"
            )
...
class DeepSeekV4StateHostPool(HostKVCache):
    ...
    External indices are SWA token indices. Internally they are converted to
    SWA page rows, where each row is one complete state ring for that SWA page.
    """
    ...
    def _init_device_page_views(self) -> None:
        ...
        if expected_ring_size is None:
            expected_ring_size = 0
            expected_state_page_bytes = 0
    ...
    def available_size(self):
        raise NotImplementedError(
            f"{self.pool_name} does not own indices; it reuses SWA transfer indices"
        )

    @synchronized
    def alloc(self, need_size: int) -> Optional[torch.Tensor]:
        raise NotImplementedError(
            f"{self.pool_name} does not allocate indices; it reuses SWA transfer indices"
        )

    @synchronized
    def free(self, indices: torch.Tensor) -> int:
        raise NotImplementedError(
            f"{self.pool_name} does not free indices; SWA owns the shared indices"
        )
